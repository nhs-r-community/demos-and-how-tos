...
p <- plot_ly(
  ...
  x = ~x,
  y = ~y,
  color = ~format(active_from, "%b-%y"),
  type = "scatter",
  mode = "markers",
  text = ~ccg_description,
  hoverinfo = "text"
)

edge_shapes <- pmap(edges_df, \(from, to) {
  c(x0, y0) %<-% filter(vertices_df, name == from)[,c("x", "y")]
  c(x1, y1) %<-% filter(vertices_df, name == to  )[,c("x", "y")]
  list(
    type = "line",
